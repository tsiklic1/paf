...
class el_magn:
    def __init__(self, polje, q, m, v, e, b):
        self.listaX = [0]
        self.listaY = [0]
        self.listaZ = [0]
    ...
    def __a_f(self, v):
        return self.q/self.m * (self.E + np.cross(v, self.polje(self.t)))

    def __move_f(self):
        k1v = self.__a_f(self.v) * self.dt
        k1xyz = self.v * self.dt 

        k2v = self.__a_f(self.v + k1v/2) * self.dt
        k2xyz = (self.v + k2v/2) * self.dt

        k3v = self.__a_f(self.v + k2v/2) * self.dt
        k3xyz = (self.v + k2v/2) * self.dt

        k4v = self.__a_f(self.v + k3v) * self.dt
